...
class IPRateLimitMiddleware(BaseHTTPMiddleware):
    ...
    - при каждом запросе:
        ...
        * если после этого количество запросов >= max_requests_per_window — отдаём 429;
        * иначе добавляем текущий timestamp и пропускаем запрос дальше.

    Предполагается, что реальный "боевой" лимит по биржевым API реализует
    отдельный модуль `src/integration/bybit/rate_limiter.py`, а это — защита
    веб-API от чрезмерно активных клиентов (UI/скриптов).
    """

    def __init__(self, app, config: Optional[RateLimitConfig] = None) -> None:  # type: ignore[override]
        super().__init__(app)
    ...
    async def dispatch(
        ...
        # Если лимит отключён — просто пробрасываем запрос.
        ...
        if len(timestamps) >= self._config.max_requests_per_window:
            ...
            # Минимальный ответ в текстовом виде; UI при желании
            # может показать более дружелюбное сообщение.
